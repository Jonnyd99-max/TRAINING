...
function App() {
  ...
  const seek = t => { setPlaying(false); audio.current?.pause(); const value=Math.max(0,Math.min(scene.duration,t)); elapsedRef.current=value;setElapsed(value);if(audio.current)audio.current.currentTime=Math.min(value,scene.audio_duration||0); };
  ...
  return <>
    ...
    {!project ? <main className="dashboard">
      ...
      </aside><section className="previewpanel"><div className="panelheading"><h2>Preview</h2><span>1920 × 1080 <b>·</b> 16:9</span></div>{scene?.kind === 'image' && scene?.image ? <VisualWorkspace key={scene.id + scene.image} scene={scene} src={mediaUrl(scene.image)} locked={locked} playing={playing} elapsed={elapsed} onChange={patchScene} onError={setError}/> : (<div className="previewstage"><div className="preview" style={{ opacity: playing ? Math.max(0, Math.min(1, elapsed/.3, (scene.duration-elapsed)/.3)) : 1 }}>
