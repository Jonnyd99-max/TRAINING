import Subtitles, {SubtitleTiming} from './Subtitles.jsx';
...
const makeScene = (voice = 'en-GB-RyanNeural') => ({ id: crypto.randomUUID().replaceAll('-', ''), kind: 'image', title: 'Untitled scene', subtitle: '', image: null, narration: '', caption: '', subtitles_enabled: true, duration: 5, manual_duration: false, voice, speed: 0, audio: null, audio_key: null, audio_duration: null });
...
function App() {
  ...
  const patchScene = values => {
    ...
    change({ ...p, video: null, scenes: p.scenes.map((s, i) => i === selected ? { ...s, ...values, ...(invalidate ? { subtitle_cues: null, audio: null, audio_key: null, audio_duration: null } : {}) } : s) });
  };
  ...
  const seek = t => { setPlaying(false); setSeekVersion(v=>v+1); audio.current?.pause(); const value=Math.max(0,Math.min(scene.duration,t)); elapsedRef.current=value;setElapsed(value);if(audio.current)audio.current.currentTime=Math.min(value,scene.audio_duration||0); };
  ...
  return <>
    ...
    {!project ? <main className="dashboard">
      ...
      <div className="offlinebar projectoffline"><span>{onlineSceneCount ? `${onlineSceneCount} scene${onlineSceneCount === 1 ? '' : 's'} use online voices. Their narration text is sent to Microsoft when generated.` : 'All scenes use offline voices. Narration stays on this computer.'}</span>{onlineSceneCount > 0 && <button disabled={locked || playing || !health?.offline_ready} onClick={() => { setPlaying(false); change({ ...project, video: null, scenes: project.scenes.map(s => s.voice.startsWith('piper:') ? s : { ...s, voice: health.offline_default_voice || 'piper:en_GB-northern_english_male-medium', audio: null, audio_key: null, audio_duration: null }) }); setNotice('Project switched to offline voices. Generate narration or export to create local audio.'); }}>Use offline voices for this project</button>}</div>
      ...
      <div className="editorgrid"><aside className="scenes"><div className="panelheading"><h2>Scenes</h2><span>{String(project.scenes.length).padStart(2, '0')}</span></div><div className="scenelist">{project.scenes.map((s, i) => <button className={`sceneitem ${i === selected ? 'selected' : ''}`} key={s.id} disabled={locked} onClick={() => select(i)}><div className="thumb">{s.image ? <img alt="" src={mediaUrl(s.image)}/> : <span>{s.kind === 'title' ? 'T' : '+'}</span>}<b>{String(i+1).padStart(2, '0')}</b></div><strong>{s.title || `Scene ${i+1}`}</strong><small>{formatTime(s.duration)} · {s.audio ? 'Narrated' : s.kind === 'title' ? 'Title' : 'No narration audio'}</small></button>)}</div><button className="addscene" disabled={locked || playing || project.scenes.length >= 100} onClick={() => { change({ ...project, video: null, scenes: [...project.scenes, makeScene(health?.default_voice)] }); select(project.scenes.length); }}>+ Add scene</button>
      ...
      </aside><section className="previewpanel"><div className="panelheading"><h2>Preview</h2><span>1920 × 1080 <b>·</b> 16:9</span></div>{scene?.kind === 'image' && scene?.image ? <VisualWorkspace key={scene.id + scene.image} scene={scene} src={mediaUrl(scene.image)} locked={locked} playing={playing} elapsed={elapsed} seekVersion={seekVersion} onChange={patchScene} onError={setError}/> : (<div className="previewstage"><div className="preview" style={{ opacity: playing ? Math.max(0, Math.min(1, elapsed/.3, (scene.duration-elapsed)/.3)) : 1 }}>
        ...
        <label>Narration text<textarea rows={5} maxLength={10000} placeholder="Explain what the learner should do…" value={scene.narration} onChange={e => patchScene({ narration: e.target.value })}/><small>{offlineScene ? 'Generated on this computer. No internet or account required.' : 'Online voice: text is sent to Microsoft Edge TTS when generated.'}</small></label><label className="checkbox"><input type="checkbox" checked={!!scene.subtitles_enabled} onChange={e => patchScene({subtitles_enabled:e.target.checked})}/>Show narration subtitles</label><p className="hint">Short phrases use estimated timing from the generated audio. Generate narration before previewing. Subtitles replace the static caption while enabled; the caption text is retained.</p><label>On-screen caption <small>OPTIONAL</small><textarea rows={2} maxLength={220} placeholder="A short, useful takeaway" value={scene.caption} onChange={e => patchScene({ caption: e.target.value })}/></label>
        <SubtitleTiming scene={scene} elapsed={elapsed} onSeek={seek} onChange={patchScene}/><label>Scene duration (seconds)<input type="number" min="1" max="3600" step="0.1" value={scene.duration} onChange={e => patchScene({ duration: Math.max(1, Math.min(3600, Number(e.target.value) || 1)), manual_duration: true })}/></label><label className="checkbox"><input type="checkbox" checked={!scene.manual_duration} onChange={e => patchScene({ manual_duration: !e.target.checked, ...(e.target.checked && scene.audio_duration ? { duration: Math.round((scene.audio_duration+1)*100)/100 } : {}) })}/>Auto duration: narration + 1 second</label>{scene.manual_duration && scene.audio_duration > scene.duration && <small className="warning">Narration will be cut short at this duration.</small>}
